...
public class BattleTest {
    ...
    @Test
    @Tag("11-23")
    @DisplayName("Test midnightArmour reduces enemy attack and add player's attack.")
    public void testMidnightArmourEffect() throws InvalidActionException {
        DungeonManiaController controller;
        controller = new DungeonManiaController();
...
//        assertEquals(2, TestUtils.getInventory(res, "wood").size());
        ...
        int enemyAttack = Integer.parseInt(Objects.requireNonNull(TestUtils.getValueFromConfigFile("spider_attack",
        config)));

        int midNightArmourDefence = Integer.parseInt(
            Objects.requireNonNull(TestUtils.getValueFromConfigFile("midNightArmourDefence", config)));

        int playerAttack = Integer.parseInt(
            Objects.requireNonNull(TestUtils.getValueFromConfigFile("player_attack", config)));

        int at = Integer.parseInt(
            Objects.requireNonNull(TestUtils.getValueFromConfigFile("midnight_armour_attack", config)));

        double expectedDamage = ((double) enemyAttack - (double) midNightArmourDefence) / 10;

        double expectedAttack = ((double) playerAttack + (double) at) / 5;

        // Delta health is negative so take negative here
        assertEquals(expectedDamage, -firstRound.getDeltaCharacterHealth(), 0.001);
        assertEquals(expectedAttack, -firstRound.getDeltaEnemyHealth(), 0.001);
    }
    @Test
    @Tag("11-16")
